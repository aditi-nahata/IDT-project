...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        sample_lands = [
            ...
            ('K-102', 'B-502', 'None (Govt Property)', 'S-100', '13.01', '77.62', 2400, 75000, 'Pending'),
            ('K-103', 'B-503', 'None (Govt Property)', 'S-101', '12.85', '77.40', 1800, 52000, 'Verified'),
            ('K-104', 'B-504', 'Diya Patel', 'S-102', '12.92', '77.51', 1500, 0, 'Pending'),
            ('K-105', 'B-505', 'None (Govt Property)', 'S-103', '13.10', '77.70', 3000, 110000, 'Verified'),
            ('K-106', 'B-506', 'None (Govt Property)', 'S-104', '12.95', '77.53', 1100, 45000, 'Verified'),
            ('K-107', 'B-507', 'None (Govt Property)', 'S-105', '13.05', '77.66', 2200, 89000, 'Pending'),
            ('K-108', 'B-508', 'Aarav Mehta', 'S-106', '12.88', '77.45', 1350, 0, 'Verified'),
            ('K-109', 'B-509', 'None (Govt Property)', 'S-107', '12.99', '77.58', 4000, 165000, 'Verified'),
            ('K-110', 'B-510', 'None (Govt Property)', 'S-108', '13.02', '77.61', 950, 38000, 'Pending'),
            ('K-111', 'B-511', 'None (Govt Property)', 'S-109', '12.81', '77.38', 2100, 72000, 'Pending'),
...
class LandRegistryApp:
    ...
    def setup_control_panel(self):
        ...
        if self.current_role == "Admin":
            ...
            # System Auditing/Audit Deletion
    ...
    def handle_row_selection(self, event):
        ...
        self.ent_price.delete(0, tk.END); self.ent_price.insert(0, values[8])
    ...
    def admin_edit_record(self):
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE id=?
            """, (self.ent_khata.get(), self.ent_bhoomi.get(), self.ent_owner.get(), self.ent_survey.get(),
                  self.ent_lat.get(), self.ent_long.get(), float(self.ent_area.get()), float(self.ent_price.get()), record_id))
            conn.commit()
            conn.close()
    ...
    def show_chart_window(self):
        ...
        canvas = tk.Canvas(graph_win, width=460, height=260, bg="white")
        canvas.pack(pady=10)
        
        canvas.create_line(50, 210, 430, 210, width=2)
        ...
        for i, (bhoomi, val) in enumerate(data):
            if i > 5: break 
            height = int((val / max_val) * 155) if max_val > 0 else 0
            ...
            canvas.create_text((x0 + x1)/2, y0 - 15, text=f"${int(val)}", font=("Arial", 8, "bold"))
            canvas.create_text((x0 + x1)/2, 225, text=bhoomi, font=("Arial", 8), angle=15)
